         ...
         size_t authenticated_length, uint8_t tag[32])
{
    ...
    if (HMAC(EVP_sha256(), key, MQVPN_RELAY_KEY_SIZE, authenticated_data,
             authenticated_length, tag, &tag_length) == NULL ||
        tag_length != 32) {
...
{
    if (datagram_length != NULL) {
        *datagram_length = 0;
    }
    if (key == NULL || datagram == NULL || datagram_length == NULL ||
        (payload_length != 0 && payload == NULL) || !valid_type(type) ||
    ...
    if (datagram_capacity < encoded_length) {
        return MQVPN_RELAY_ERR_OUTPUT_TOO_SMALL;
    }
...
{
    if (key == NULL || datagram == NULL || frame == NULL ||
        !valid_direction(expected_direction)) {
        return MQVPN_RELAY_ERR_INVALID_ARGUMENT;
    }
    ...
    if (datagram[4] != MQVPN_RELAY_VERSION) {
        return MQVPN_RELAY_ERR_UNSUPPORTED_VERSION;
    }
    mqvpn_relay_message_type_t type = (mqvpn_relay_message_type_t)datagram[5];
    if (!valid_type(type)) {
        return MQVPN_RELAY_ERR_UNKNOWN_TYPE;
    }
    mqvpn_relay_direction_t direction = (mqvpn_relay_direction_t)datagram[6];
    if (!valid_direction(direction)) {
        return MQVPN_RELAY_ERR_INVALID_DIRECTION;
    }
    ...
    if (payload_length > MQVPN_RELAY_MAX_PAYLOAD_SIZE ||
        datagram_length > MQVPN_RELAY_MAX_DATAGRAM_SIZE) {
        return MQVPN_RELAY_ERR_PAYLOAD_TOO_LARGE;
    }
    const size_t expected_length =
        MQVPN_RELAY_HEADER_SIZE + payload_length + MQVPN_RELAY_TAG_SIZE;
    if (datagram_length != expected_length) {
        return MQVPN_RELAY_ERR_LENGTH_MISMATCH;
    }
    ...
    if (direction != expected_direction) {
        return MQVPN_RELAY_ERR_WRONG_DIRECTION;
    }
    if (expected_session != NULL && session_id != *expected_session) {
...
mqvpn_replay_window_accept(mqvpn_replay_window_t *window, uint64_t sequence)
{
    ...
    if (age >= 64) {
        return MQVPN_RELAY_ERR_REPLAY;
    }
    uint64_t sequence_bit = UINT64_C(1) << age;
    if ((window->received_bitmap & sequence_bit) != 0) {
        return MQVPN_RELAY_ERR_REPLAY;
    }

...
/* Multipath scheduling contract for this lane — unpinned weighted WLB.
 *
 * Lane bytes travel as QUIC STREAM frames, and xquic sets po_flow_hash only
 * on the datagram write path (xqc_packet_out.c), so lane packets reach WLB
 * with hash 0. WLB identifies STREAM frames separately and schedules their
 * data per packet using live path weights; ACK/control-only packets retain
 * the MinRTT fallback.
 *
 ...
 * that reordering before any TCP stack can see it. Flow pinning would cap a
 * single Hybrid flow near one path's capacity, while weighted per-packet
 * scheduling lets it consume the aggregate send window and automatically
 * biases capacity toward the stronger path. */
